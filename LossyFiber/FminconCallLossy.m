function [] = FminconCallLossy(cp, N, E, Tp,alpha, beta2, gamma, L, epsilon)
...
if (sum(v.^2.*(1-Lambdas(1:length(v))))>epsilon*sum(v.^2))
    ...
    try %compare with previous result if it exists
        ...
        if(results(end)>TpTilde*10^-9)% if new result is better than previous one, then update results
            ...
            save(strcat('results/Alphas_Lossy_E',num2str(E*1e15),'fJ_Tp',num2str(Tp*1e12),'ps_N', num2str(N),'cp',num2str(cp),'_L',num2str(L*1e-3),'Km.mat'),'results');
        end
    catch
